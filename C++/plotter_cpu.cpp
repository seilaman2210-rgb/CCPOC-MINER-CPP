...
static bool create_plot_file(const std::string& plot_path, const std::string& plot_id,
                              ...
                              bool resume = false, int thread_count = 0) {
    ...
    std::cout << "[PLOT] Using " << workers << " thread" << (workers == 1 ? "" : "s") << std::endl;
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Usage: plotter <size_gb> [threads] [output_dir] [--miner <address>] [--private-key <hex>]" << std::endl;
        std::cout << "  threads: number of CPU threads to use (default: auto-detect, capped at 64)" << std::endl;
        std::cout << "Auto-reads wallet.json if present (same dir as executable, ./wallet.json, or wallet.json)" << std::endl;
        std::cout << "Example: plotter 1 8 ./plots" << std::endl;
        return 1;
    }
    ...
    if (positional.empty()) {
        std::cerr << "[PLOT] Missing <size_gb> argument" << std::endl;
        return 1;
    }
    double size_gb = std::atof(positional[0].c_str());
    int thread_count = (positional.size() >= 2) ? std::atoi(positional[1].c_str()) : 0;
    if (thread_count < 0) thread_count = 0;
    std::string outdir = (positional.size() >= 3) ? positional[2] : "./plots";
    ...
    bool ok = create_plot_file(plot_path, plot_id, miner_address, size_gb, account_id, true, false, thread_count);
