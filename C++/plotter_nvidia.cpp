...
#include "gpu_modules/nvidia_cuda.h"
...
static const char* gpu_backend_name() {
    return g_gpu_ready ? "NVIDIA CUDA" : "CPU";
}

static bool gpu_generate_scoops_dispatch(const uint8_t* account_id, uint32_t nonce, uint32_t count,
                                          uint32_t total_scoops, std::vector<std::vector<uint8_t>>& results) {
    if (!g_gpu_ready) return false;
    return cuda_generate_scoops(account_id, nonce, count, total_scoops, results);
}

static void gpu_shutdown_active() {
    if (g_gpu_ready) {
        shutdown_nvidia_cuda();
    }
}
...
static bool create_plot_file(const std::string& plot_path, const std::string& plot_id,
    ...
    if (start_stage.empty() || start_stage == "HASH") {
        ...
        if (use_gpu) {
            std::cout << "  [HASH] Using " << gpu_backend_name() << " GPU for SHA-256 hashing..." << std::endl;
        }
        ...
        while (leaf_count < total_scoops) {
            ...
            if (use_gpu) {
                ...
                if (gpu_generate_scoops_dispatch(account.data(), static_cast<uint32_t>(first_nonce + leaf_count / SCOOPS_PER_NONCE), batch, total_scoops, gpu_results)) {
...
int main(int argc, char* argv[]) {
    ...
    g_gpu_ready = is_nvidia_cuda_available();
    if (gpu_available()) {
        std::cout << "[PLOT] " << gpu_backend_name() << " GPU enabled for hashing" << std::endl;
    } else {
        std::cout << "[PLOT] No supported GPU found, using CPU" << std::endl;
    }
